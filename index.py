...
def scrape_leaderboard(url, login_url, username, password):
    ...
    #Using JavaScript to set the value for username
    ...
    #Using Javascript to click continue button
    ...
    #Using JavaScript to set the value for password
    driver.execute_script(
    ...
    #Navigating to the competition URL
    driver.get(url)
    ...
    #Finding ranking table
    table = soup.find('table', {'class': 'table table--primary ranking'})
    
    time.sleep(2)

    #clicking 'next page' to go to page 2 of the rankings
    ...
    tables = [table, table2]
    all_rows = []
    #Processing each table, extracting headers 
    for table in tables:
        if table:
            #Extracting headers
...
def compare(lastWeek: pd.DataFrame, thisWeek: pd.DataFrame):
    #Intersecting the dataframes on the Name column
...
#dropping unnecessary fields from the merged df
